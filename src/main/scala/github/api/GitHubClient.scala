...
class GitHubClient(private val http: Client[IO], private val config: Config.GitHub)
    ...
    paginatedGetRequest(
      ...
      customStatusCheck = {
        case Status.NoContent(_) =>
          // IO(logger.warn(s"Organization '$organization' has no repositories")) *>
          IO(List.empty)
  ...
  override def contributorsOfRepository(
    ...
    paginatedGetRequest(
      ...
      customStatusCheck = {
        case Status.NoContent(_) =>
          // IO(logger.warn(s"Repository '$organization/$repository' has no contributors")) *>
          IO(List.empty)
  ...
  /**
    ...
    * @param uri
    *   URI of the request to make
    * @param page
    *   Number of the page requested, used in pagination
    * @param lastPage
    *   Number of the last page (if available), used in creating subsequent requests in pagination
    * @param customStatusCheck
    *   Partial function to perform on the response to handle status-code specific use cases
    * @param parse
    *   Function to parse the response of successful requests to desired type
    *
    * @tparam A
...
object GitHubClient {
  ...
  /**
    ...
    *
    * @return
    *   Some failed IO with rate limit error when rate limit is hit, None otherwise
    */
  ...
  /**
    ...
    *   An HTTP response
    * @param log
    *   Log message describing the action that failed
    * @param maybeCause
    *   An optional error that caused the failure
    *
    * @tparam A
    *   Type of the requested value
    *
