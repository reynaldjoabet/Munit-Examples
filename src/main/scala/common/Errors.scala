...
object Errors {
  ...
  /**
    ...
    * @param maybeCause
    *   An optional cause of the failure
    */
  sealed abstract class APIError(val message: String, val maybeCause: Option[Throwable] = None)
      extends Exception(message, maybeCause.orNull, false, maybeCause.isDefined) {
    ...
    /**
      ...
      *
      * @return
      *   HTTP response created by this error
      */
  ...
  /**
    ...
    *
    * @param what
    *   Identifier of the thing that's not found
    * @param kind
    *   Type of the thing that's not found
    */
  sealed abstract class NotFoundError(val what: String, val kind: String)
      extends APIError(
  ...
  /**
    ...
    *
    * @param resetInstant
    *   Instant when the rate limit will be lifted
    */
  case class RateLimited(resetInstant: Instant)
      extends APIError(
  ...
  /**
    ...
    */
  case class OrganizationNotFound(organization: String)
      extends NotFoundError(organization, "organization")
  ...
  /**
    ...
    *   An optional cause of the failure
    * @param details
    *   Optionally some details about the error (error message returned from API if available)
    */
  case class Unavailable(override val maybeCause: Option[Throwable], details: Option[String] = None)
      extends APIError(
        s"Service is unavailable${details.fold("")(d => s", details: $d")}",
        maybeCause
      ) {
  ...
  /**
    ...
    *   Key of the config
    * @param value
    *   Value of the config that is invalid
    */
  case class InvalidConfig(config: String, value: Any)
      extends APIError(
