...
//  }
  ...
  val clientRoute = HttpRoutes.of[IO] {
    ...
    case GET -> Root / "client" / "todos" =>
      val out: IO[String] = EmberClientBuilder.default[IO].build.use { client =>
        val url = s"$HOST/todos"
        client.expect[String](url)
      }
      out.flatMap(str => Ok(str))
    case GET -> Root / "client" / "todos" / IntVar(id) =>
      val stringIO: IO[String] = EmberClientBuilder.default[IO].build.use { client =>
    ...
    case GET -> Root / "client" / "posts" =>
      ...
      val postsIO = EmberClientBuilder.default[IO].build.use { client =>
    ...
    case GET -> Root / "client" / "posts" / IntVar(id) =>
      import org.http4s.circe.CirceEntityCodec.circeEntityDecoder
      import org.http4s.circe.CirceEntityCodec.circeEntityEncoder
      val postIO = EmberClientBuilder.default[IO].build.use { client =>
        val url = s"$HOST/posts/$id"
        client.expect[Post](url)
      }

      // on error (of any type) we simply return some dummy post
      postIO.onError(th=> Logger[IO].error(s"***Error*** $th")).orElse {
        IO(Post(id, 1000, "", ""))
    ...
    case GET -> Root / "client" / "twiceslow" =>
      val outTuple = EmberClientBuilder.default[IO].build.use { client =>
        val url = "http://localhost:8080/slow"
  ...
  val run= {
    ...
    for {
      ...
      _ <- server.EmberServerBuilder.default[IO]
